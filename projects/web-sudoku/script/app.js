...
function generateBoard(board) {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    let row = Math.floor(i / boardSize) + 1;
    let col = (i % boardSize) + 1;
    if (col % boxSize == 0 && col != boardSize) {
      tile.classList.add("rightBorder");
    }
    if (row % boxSize == 0 && row != boardSize) {
...
function updateMove() {
  if (selectedTile && selectedNum) {
    ...
    if (isCorrect(selectedTile)) {
      ...
      currentBoard[Math.floor(selectedTile.id / boardSize)][
        selectedTile.id % boardSize
...
function show_solution() {
  ...
  for (let i = 0; i < id("board").children.length; i++) {
    ...
    if (tile.textContent != solution[tile.id]) {
      ...
      // Update the curernt status of Sudoku board
      currentBoard[Math.floor(tile.id / boardSize)][tile.id % boardSize] =
        solution[tile.id];
